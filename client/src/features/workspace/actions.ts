...
export const deleteFD = (relationId: TableId, index: number) =>
  makeAction((ws) => ({
    ...ws,
    relations: updateRelation(ws.relations, relationId, (r) => ({
      ...r,
      fds: r.fds.filter((_, i) => i !== index),
    })),
  }));
...
export const deleteCrossTableFD = (index: number) =>
  makeAction((ws) => ({
    ...ws,
...
export const mergeRelations = (id1: TableId, id2: TableId) =>
  makeAction((ws) => {
    ...
    if (Option.isNone(r1Opt) || Option.isNone(r2Opt)) return ws;
    ...
    return {
      ...
      crossTableFDs: ws.crossTableFDs.filter((c) => c.fromTableId !== id2 && c.toTableId !== id2),
