...
import { TreeNode, TableHealth, OptimizationStrategy } from "../workspace/model";
    ...
    return (
        <div className="flex flex-col gap-2 ml-4">
            <div className="flex items-center gap-2 border-l-2 border-slate-200 pl-2 py-1">
                <Badge variant="outline" className="text-xs font-mono">{node.relation.name}</Badge>
                <span className="text-xs text-slate-400">
                    ({node.relation.attributes.join(", ")})
                </span>
            </div>
...
export const OptimizerView: React.FC<OptimizerViewProps> = ({
    ...
    const handleSliderChange = (vals: number[]) => {
        const v = vals[0];
        if (v < 33) onStrategyChange('bcnf');
        else if (v < 66) onStrategyChange('3nf');
        else onStrategyChange('performance');
    };
        ...
        <Sheet open={isOpen} onOpenChange={onOpenChange}>
            <SheetContent className="sm:max-w-md overflow-y-auto">
                <SheetHeader>
                    ...
                    <SheetDescription>
                        Optimize table structure for {targetRelationId}
                    </SheetDescription>
                </SheetHeader>
                        ...
                        {Option.isSome(latestAnalysis) ? (
                            <div className="flex flex-col gap-3">
                                {Option.map(latestAnalysis.value.health, h => (
                                    <div key="health" className={`p-2 rounded border ${h.severity === 'error' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'}`}>
                                ...
                                {latestAnalysis.value.warnings.length > 0 && (
                                    <div className="text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-100">
                                        ...
                                        <ul className="list-disc list-inside">
                                            {latestAnalysis.value.warnings.map((w, i) => <li key={i}>{w}</li>)}
                                        </ul>
                                    </div>
                                )}
                                ...
                                {Option.map(latestAnalysis.value.tree, tree => (
                                    <div key="tree" className="mt-2">
                                        <div className="text-xs font-bold text-slate-500 uppercase mb-2">Decomposition Tree</div>
                            ...
                            <p className="text-slate-500 italic">No analysis results yet. Run Auto-Fix to analyze.</p>
                        )}
                    </div>
                </div>
